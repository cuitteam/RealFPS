...
/**
 * 
 * @author Silent Show welcome page with some views for user to interactive
 */
public class WelcomePageHandler extends ActivityInitHandler {
	...
	public void initParamters() {
		sp.setAdapter(new ArrayAdapter(activity,
				android.R.layout.simple_dropdown_item_1line, scanProcessList()));
	}

	/**
	 * 
	 * @return prepare the processList of device
	 */
	private List<Object> scanProcessList() {
		...
		List<RunningAppProcessInfo> runningProcess = mActivityManager
		...
		List<PackageInfo> intsllPacks = activity.getPackageManager()
		...
		for (int i = 0; i < intsllPacks.size(); i++) {
			...
			packInfo.put(p.packageName, p.applicationInfo.loadLabel(pkMag)
					.toString());
		}
